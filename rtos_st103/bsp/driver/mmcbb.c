...
#define DLY_US(n)	taskDelay(0)	/* Delay n microseconds */

#define	CS_H()		the_sd_spi->select(the_sd_spi, FALSE)	/* Set MMC CS "high" */
#define CS_L()		the_sd_spi->select(the_sd_spi, TRUE)	/* Set MMC CS "low" */
...
)
{
    ...
    {
       (void)the_sd_spi->send(the_sd_spi, buff[i]);
    }
}
...
)
{
    for (uint16_t i = 0; i < bc; i++)
    {
        buff[i] = the_sd_spi->send(the_sd_spi, 0xff);
    }
}
...
)
{
	...
	if (sd_status() & STA_NOINIT) return RES_NOTRDY;	/* Check if card is in the socket */
